...
class Usuario extends Model {
	...
	public function validarCadastro() {
		$valido = true;
		$errorType = array(1 => 'errnome', 2 => 'erremail', 3 => 'errsenha');
	...
	public function getAll(){
		...
		from
			usuarios as u
		where
			nome like :nome and u.id != :id_usuario
		';

		$stmt = $this->db->prepare($query);
		// % qualquer coisa a esquerda ou a direita
		// qualquer palavra que contenha a aparece
		$stmt->bindValue(':nome', '%'.$this->__get('nome'). '%');
	...
	public function seguirUsuario($id_usuario_seguir){
		...
		$stmt = $this->db->prepare($query);
		$stmt->bindValue(':id_usuario', $this->__get('id'));
		$stmt->bindValue(':id_usuario_seguindo', $id_usuario_seguir);
		$stmt->execute();
	...
	public function removerTweetUsuario($id_tweet){
        $query = '
            DELETE FROM tweets
            WHERE id = :id_tweet
	...
	//quantidade de usuarios que estou seguindo
